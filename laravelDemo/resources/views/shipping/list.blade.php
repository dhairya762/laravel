...
@section('content')
    ...
    <input type="button" value="Add" class="btn btn-success"
        onclick="mage.setUrl('shipping/add/0').setMethod('get').load();">
    <br><br>
    <div class="row">
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="button" class="btn btn-success"
                                        onclick="mage.setUrl('shipping/add/{{ $shipping->id }}').setMethod('get').load();"
                                        value="Edit">
                                </td>
                                <td>
                                    <input type="button" class="btn btn-danger"
                                        onclick="mage.setUrl('shipping/destroy/{{ $shipping->id }}').setMethod('get').load();"
                                        value="Delete">
                                </td>
                            </tr>
                        @endforeach
                    ...
                    <div class="col-md-6">
                        <div class="form-group ">
                            ...
                            <select name="records_per_page" id="records_per_page" onchange="mage.setForm('paginate')">
                                <option value="Select" {{ session('paginate') < 3 ? 'selected' : '' }}>Select
                                </option>
        ...
        <div class="col-md-6">
            @for ($i = 1; $i <= $totalPage; $i++)
                <a href="javascript:void(0)" name="page" value="{{ $i }}"
                    onclick="mage.setUrl('shipping/page={{ $i }}').setMethod('get').load();"
                    id="{{ $i }}" class="btn btn-primary">{{ $i }}</a>
            @endfor
        </div>
    </div>
@endsection